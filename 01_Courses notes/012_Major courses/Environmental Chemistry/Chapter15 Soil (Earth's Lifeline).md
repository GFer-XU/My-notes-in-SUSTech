...
```mermaid
graph LR
    A[Soil: Earth's Lifeline] --> B[Structure & Composition]
    A --> C[Acid-Base & Ion-Exchange]
    A --> D[Nutrients & Fertilizers]
    A --> E[Pollutants & Wastes]
    A --> F[Degradation & Restoration]

    B --> B1[Regolith: O, A, E, B, C Horizons]
    B --> B2[Minerals, Water, Air Spaces]
    B --> B3[Organic Matter / Humus]

    C --> C1[Acidification: Pyrite Oxidation]
    C --> C2[Neutralization: Limestone]
    C --> C3[Cation & Anion Exchange Equilibria]

    D --> D1[Macronutrients: N, P, K, Ca, Mg, S]
    D --> D2[Micronutrients: Fe, Mn, Zn, Cu, etc.]
    D --> D3[Fertilizers: Haber-Bosch Process]
    D --> D4[Eutrophication Dead Zones]

    E --> E1[Pesticides & Fumigants]
    E --> E2[Biodegradation in Rhizosphere]
    E --> E3[Black Carbon Residues]

    F --> F1[Erosion, Desertification, Deforestation]
    F --> F2[Conservation: Terracing, Alley Cropping]
    F --> F3[Restoration: Phytoremediation & Poduculture]
```
