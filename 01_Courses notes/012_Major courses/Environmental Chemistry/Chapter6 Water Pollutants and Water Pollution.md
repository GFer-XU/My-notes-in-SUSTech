...
```mermaid
graph LR
    A[Water Pollutants & Pollution] --> B[Elemental Pollutants]
    A --> C[Inorganic Species]
    A --> D[Nutrients & Bulk Parameters]
    A --> E[Oxygen Demand Indicators]
    A --> F[Organic Pollutants]
    A --> G[Radionuclides]

    B --> B1[Heavy Metals: Cd, Pb, Hg]
    B --> B2[Metalloids: As, Se, Sb]
    B --> B3[Organically Bound Metals]

    C --> C1[Cyanide HCN]
    C --> C2[Ammonia & Free CO2]
    C --> C3[H2S, Nitrites, Sulfites]
    C --> C4[Asbestos & Perchlorates]

    D --> D1[Eutrophication & Algal Nutrients]
    D --> D2[Acidity, Alkalinity, Salinity]

    E --> E1[DO: Dissolved Oxygen]
    E --> E2[BOD & COD]
    E --> E3[TOC: Total Organic Carbon]

    F --> F1[Sewage & Microbial Toxins]
    F --> F2[Soaps & Detergents]
    F --> F3[Pesticides: Insecticides, Herbicides, Fungicides]
    F --> F4[POPs & PCBs]
    F --> F5[Emerging Pollutants: Pharmaceuticals, Siloxanes]

    G --> G1[Types: Alpha, Beta, Gamma]
    G --> G2[Decay Kinetics & Half-life]
```
