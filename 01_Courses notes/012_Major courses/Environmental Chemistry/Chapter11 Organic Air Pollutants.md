...
```mermaid
graph LR
    A[Organic Air Pollutants] --> B[Atmospheric Loss Mechanisms]
    A --> C[Biogenic Organics]
    A --> D[Hydrocarbon Pollutants]
    A --> E[Heteroatom Organics]
    A --> F[Particulate Matter & POPs]

    B --> B1[Wet & Dry Deposition]
    B --> B2[Photochemical Reactions]
    B --> B3[Plant Uptake]

    C --> C1[Methane from Anoxic Bacteria]
    C --> C2[Terpenes from Vegetation]

    D --> D1[Alkanes: Stable, Abstraction]
    D --> D2[Alkenes/Alkynes: Reactive, Addition]
    D --> D3[Aromatics: Ring Addition]

    E --> E1[Carbonyls: Aldehydes & Ketones]
    E --> E2[Oxygen-containing: Alcohols, Phenols, Ethers]
    E --> E3[Nitrogen-containing: Amines, Nitro, Nitriles]
    E --> E4[Sulfur-containing: Thiols, Thioethers]
    E --> E5[Halides: CFCs, Halons, Alkyl/Aryl Halides]

    F --> F1[Secondary Organic Aerosols SOA]
    F --> F2[Persistent Organic Pollutants POPs]
    F --> F3[Global Distillation Effect]
```
