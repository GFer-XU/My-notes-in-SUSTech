...
```mermaid
graph LR
    A[Atmospheric Chemistry & Climate Change] --> B[Photochemical Smog]
    A --> C[Global Warming]
    A --> D[Acid Rain]
    A --> E[Stratospheric Ozone Depletion]
    A --> F[Aerosols & Brown Clouds]

    B --> B1[Key Ingredients: UV, HCs, NOx]
    B --> B2[Radical Chain Reactions: Abstraction & Addition]
    B --> B3[Oxidants: O3, PAN, Aldehydes]

    C --> C1[Outgoing Infrared Absorption]
    C --> C2[GHGs: CO2, CH4, N2O, Halocarbons]

    D --> D1[Precursors: SO2, NOx]
    D --> D2[Acids: H2SO4, HNO3]
    D --> D3[Impacts: Phytotoxicity, Stone Deterioration]

    E --> E1[UV Protection: 220-330 nm]
    E --> E2[CFC Photolysis & Cl• Catalyst]
    E --> E3[Antarctic Ice Cloud Reactions]

    F --> F1[Urban Aerosols & Brown Clouds]
    F --> F2[Nuclear Winter / Volcanic Cooling]
```
